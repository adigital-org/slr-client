  ...
  switch(type) {
    ...
    case 'session/done':
      ...
      taskToWorker(workers[workerMsg.workerId-1], workerParams, params.linesPerTask)
        .catch(() => {throw new Error('Fatal error feeding worker')})
      break
  }
}
...
const finish = async (code, errMsg) => {
  try {
    ...
    process.stderr.write('\nError cleaning session. Some results may have been lost.')
    process.exit(1)
  }
}
...
const setupMaster = async () => {
  ...
  params.totalRecords = await countLines(params.inputCsv)
